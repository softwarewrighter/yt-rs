...
async fn stream_video(
    ...
    match range {
        Some((start, end)) => build_partial_response(file, start, end, file_size).await,
        None => Ok(build_full_response(file, file_size)),
    }
}
...
fn build_full_response(file: File, file_size: u64) -> Response {
